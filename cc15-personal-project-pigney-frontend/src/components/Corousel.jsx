...
function Corousel() {
    const {dataGoal, dataGoalRecord} = useAuth();
    ...
    dataGoal.myGoal.map(x => {
      ...
      let status = "";
      let obj = {};
      dataGoalRecord.piggyAndRecord.map(y => y.piggy.id === x.id ? y.status === "ADD" ? sum += y.fund : sum -= y.fund: y);
    ...
    slides = dataGoal.myGoal.map((x, idx) => [...slides, <PiggyGoal sum={arrSum[idx]} key={x.id} {...x} />]);
    }
