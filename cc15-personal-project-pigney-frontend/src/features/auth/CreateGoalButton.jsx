...
function CreateGoalButton({ type }) {
  ...
  let context = (
    <div className="text-black font-medium text-xs">
      <button
        onClick={handleClick}
        className="flex items-center gap-2 rounded-full px-4 py-2 bg-pigney-lemon shadow-pigney-button hover:shadow-pigney-lemon/30 duration-200"
      >
        <div>
  ...
  type === "outline" &&
    (context = (
      <div className="text-white font-medium text-xs">
        <button
          onClick={handleClick}
          className="flex items-center gap-2 border-2 rounded-full px-4 py-2 shadow-pigney-button shadow-white/0 hover:shadow-white/20 duration-200"
        >
          <div>
            <div>CREATE PIGGY</div>
            <div className="flex">GOALS</div>
